...
(function () {
  ...
  /* ===========================================================
     2. TYPED ANIMATION (Using Typed.js library)
     =========================================================== */
  function initTyped() {
    const el = document.getElementById("typed-text");
    if (!el || typeof Typed === 'undefined') {
      // Fallback to manual typing if Typed.js not loaded
      initManualTyped();
      return;
    }
    ...
    filterBtns.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        document.querySelectorAll('.project-card').forEach(card => {
          const cardLang = card.querySelector('.project-tag')?.textContent.toLowerCase() || '';
          ...
          if (filter === 'all') {
            ...
            shouldShow = cardTitle.includes('ai') || cardTitle.includes('ml') || 
                        cardTitle.includes('machine learning') || cardDesc.includes('ai') ||
                        cardDesc.includes('machine learning') || cardDesc.includes('nlp');
          } else if (filter === 'web') {
            shouldShow = cardLang.includes('html') || cardLang.includes('css') || 
                        cardLang.includes('javascript') || cardLang.includes('web');
          } else {
            shouldShow = cardLang.includes(filter);
          }
  ...
  function openProjectModal(card) {
    ...
    const description = card.querySelector('.project-top p')?.textContent || '';
    const tags = Array.from(card.querySelectorAll('.project-tag')).map(tag => tag.outerHTML).join('');
    const githubLink = card.querySelector('.project-btn.github')?.href || '#';
    const demoLink = card.querySelector('.project-btn.demo')?.href || null;
    
    // Populate modal
    document.getElementById('modal-title').textContent = title;
    document.getElementById('modal-description').innerHTML = `<p>${escHtml(description)}</p>`;
    document.getElementById('modal-tech-stack').innerHTML = tags;
    ...
    // Show basic stats (you can enhance this by fetching from GitHub API)
    document.getElementById('modal-stats').innerHTML = `
      <div class="modal-stat">
        <strong><i class="fab fa-github"></i></strong>
        <span>View on GitHub for stats</span>
      </div>
    `;
